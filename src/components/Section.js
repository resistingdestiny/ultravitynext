...
function Section(props) {
  ...
  const { bgColor = 'default', bgImage, bgImageOpacity, size = 'normal', sx = [], children, ...otherProps } = props
  ...
  const color = backgroundColor === 'transparent' ? 'inherit' : theme.palette.getContrastText(backgroundColor)

  // Get border color based on `backgroundColor`
  const borderColor = backgroundColor === 'transparent' ? false : emphasize(backgroundColor, 0.09)
